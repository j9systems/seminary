...
-- ────────────────────────────────────────────────────────────────────────────
-- Onboarding RPCs (SECURITY DEFINER so a not-yet-member can discover a class
-- and claim a roster spot without loosening the table policies)
-- ────────────────────────────────────────────────────────────────────────────

-- Create a class and become its teacher. One profile per user.
create or replace function public.create_class(p_name text, p_teacher_name text)
...
begin
  ...
  insert into public.classes (name, teacher_name)
    values (coalesce(nullif(p_name, ''), 'Seminary Class'),
            coalesce(nullif(p_teacher_name, ''), 'Teacher'))
    returning id into v_class;
  insert into public.profiles (class_id, user_id, full_name, role, color, email)
    values (v_class, auth.uid(), coalesce(nullif(p_teacher_name, ''), 'Teacher'),
            'teacher', '#5A50E6', (select email from auth.users where id = auth.uid()));
  return v_class;
end $$;

-- Classes that have at least one unclaimed student roster spot (join screen).
create or replace function public.open_classes()
returns table (id uuid, name text, teacher_name text, open_spots bigint)
language sql security definer set search_path = public as $$
  select c.id, c.name, c.teacher_name, count(p.id) as open_spots
  from public.classes c
  join public.profiles p
    on p.class_id = c.id and p.user_id is null and p.role = 'student'
  group by c.id, c.name, c.teacher_name
  order by c.name
$$;

-- Unclaimed student roster for a class.
create or replace function public.unclaimed_roster(p_class uuid)
returns table (id uuid, full_name text)
language sql security definer set search_path = public as $$
  select id, full_name from public.profiles
  where class_id = p_class and user_id is null and role = 'student'
  order by full_name
$$;

-- Claim an unclaimed roster spot as the current user.
create or replace function public.claim_profile(p_profile uuid)
returns uuid language plpgsql security definer set search_path = public as $$
begin
  if auth.uid() is null then raise exception 'not authenticated'; end if;
  if exists (select 1 from public.profiles where user_id = auth.uid()) then
  ...
  update public.profiles
    ...
    where id = p_profile and user_id is null;
  if not found then raise exception 'spot not available'; end if;
  return p_profile;
end $$;

-- Teacher adds a student roster entry.
create or replace function public.add_student(p_class uuid, p_name text, p_color text)
returns uuid language plpgsql security definer set search_path = public as $$
declare v_id uuid;
begin
  if not public.is_class_teacher(p_class) then raise exception 'not the teacher'; end if;
  insert into public.profiles (class_id, full_name, role, color)
    values (p_class, p_name, 'student', coalesce(nullif(p_color, ''), '#5A50E6'))
    returning id into v_id;
  return v_id;
end $$;

grant execute on function public.create_class(text, text)      to authenticated;
grant execute on function public.open_classes()                to authenticated;
grant execute on function public.unclaimed_roster(uuid)        to authenticated;
